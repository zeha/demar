...
def main():
    ...
    for arch in ARCHS:
        bin_pkgs.update(find_bin_pkgs_with_paths(pathlib.Path(f"{mirror}/dists/sid/main/Contents-{arch}.gz")))

    for arch in ARCHS:
        pkglist_file = pathlib.Path(f"{mirror}/dists/sid/main/binary-{arch}/Packages.gz")
